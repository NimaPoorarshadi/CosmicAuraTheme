<div align="center">


# Cosmic Aura Theme for VS Code

<p align="center">
  <a href="https://github.com/NimaPoorarshadi/CosmicAuraTheme/releases/latest">
    <img src="https://img.shields.io/github/v/release/NimaPoorarshadi/CosmicAuraTheme?style=for-the-badge&logo=github&logoColor=white&colorA=2b303b&colorB=00e8c6" alt="Latest Release">
  </a>
  <a href="https://code.visualstudio.com/updates/v1_103">
    <img src="https://img.shields.io/badge/VS_Code-v1.103+-373277.svg?style=for-the-badge&logo=visual-studio-code&logoColor=white&colorA=2b303b&colorB=7cb7ff" alt="VS Code Version">
  </a>
  <a href="https://marketplace.visualstudio.com/items?itemName=NimaPoorarshadi.cosmic-aura">
    <img src="https://img.shields.io/visual-studio-marketplace/d/NimaPoorarshadi.cosmic-aura?style=for-the-badge" alt="Download Count">
  </a>
  <a href="https://marketplace.visualstudio.com/items?itemName=NimaPoorarshadi.cosmic-aura">
    <img src="https://img.shields.io/visual-studio-marketplace/stars/NimaPoorarshadi.cosmic-aura?style=for-the-badge&logo=reverbnation&logoColor=white&colorA=2b303b&colorB=FFE66D" alt="Marketplace Rating">
  </a>
</p>

</div>

---

## 🌌 Three Variations. One Universe.

> **NOTE**: The italic keywords in all Cosmic Aura variations look especially good with the **Comic Code** font.

---

### ✦ Cosmic Aura — Aurora
> The original. Electric cyan, hot magenta, vivid lime — deep indigo grounds with neon colors dancing like the northern lights.

![Cosmic Aura Aurora JavaScript](images/Javascript.png)
![Cosmic Aura Aurora Python](images/Python.png)

---

### 🔥 Cosmic Aura — Amber Synthwave
> A retro-80s variation. Warm amber-brown dark grounds, dusty earthy accents — like coding on a late-night CRT arcade screen.

![Cosmic Aura Amber Synthwave edition 1](images/cosmic_aura_synthwave_edition_1.png)
![Cosmic Aura Amber Synthwave edition 2](images/cosmic_aura_synthwave_edition_2.png)

---

### 🌠 Cosmic Aura — Nebula
> The darkest variation. Deep purple near-black grounds inspired by Monokai Pro, with a full nebula palette blooming out of the dark.

![Cosmic Aura Nebula edition 1](images/cosmic_aura_nebula_edition_1.png)
![Cosmic Aura Nebula edition 2](images/cosmic_aura_nebula_edition_2.png)

---

## Features

- Three distinct variations selectable from the Color Theme picker
- Deep cosmic backgrounds with vibrant highlights
- Balanced contrast for long coding sessions
- Syntax highlighting for **C++, Python, JavaScript, and JSON**
- Clean UI elements with subtle shadows

## Installation

1. Open VS Code
2. Go to Extensions (`Ctrl+Shift+X` or `Cmd+Shift+X`)
3. Search for **Cosmic Aura**
4. Click **Install**
5. Go to **Color Theme** (`Ctrl+K Ctrl+T`) and choose your variation:
   - **Cosmic Aura — Aurora**
   - **Cosmic Aura — Amber Synthwave**
   - **Cosmic Aura — Nebula**
6. 🌟 Enjoy and [Rate five-stars](https://marketplace.visualstudio.com/items?itemName=NimaPoorarshadi.cosmic-aura&ssr=false#review-details).

## Why Cosmic Aura?

Whether you're navigating galaxies of C++, exploring Python nebulae, or structuring JSON constellations — **Cosmic Aura** delivers a theme series that feels truly out of this world. Three variations, one universe. 🚀

## License

[MIT](https://github.com/NimaPoorarshadi/CosmicAuraTheme/blob/master/LICENSE.md)
